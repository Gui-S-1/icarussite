...
const state = {
 ...
 dashboardMonth: null, // null = mes atual, ou 'YYYY-MM'para mes especifico
 ...
 dieselStats: null,
 dieselPeriod: 'month',
 dieselSelectedMonth: null, // null = periodo atual, ou 'YYYY-MM'para mes especifico
 // Generator Control
 generatorRecords: [],
 generatorStats: null,
...
async function initPushNotifications() {
 ...
 console.log('Push Notifications: Nao esta em ambiente nativo');
 return;
 }

 const { PushNotifications } = Capacitor.Plugins;
 if (!PushNotifications) {
 console.log('Push Notifications: Plugin nao disponivel');
 return;
 }
 ...
 console.log('Push Permissions apos request:', permStatus.receive);
 }

 if (permStatus.receive !== 'granted') {
 console.log('Push Notifications: Permissao negada');
 showNotification('Ative as notificacoes nas configuracoes do app', 'warning');
 return;
 }

 // Registrar para receber notificacoes
 await PushNotifications.register();
 console.log('Push Notifications: Registrando...');

 // Listener quando o token e recebido
 ...
 showNotification('Erro ao registrar notificacoes: '+ (error.error || 'desconhecido'), 'error');
 });

 // Listener quando notificacao e recebida (app aberto)
 await PushNotifications.addListener('pushNotificationReceived', (notification) => {
 console.log('Notificacao recebida:', notification);
 // Mostrar notificacao in-app
 ...
 // Listener quando usuario toca na notificacao
 await PushNotifications.addListener('pushNotificationActionPerformed', (action) => {
 console.log('Acao na notificacao:', action);
...
async function registerPushToken(token) {
 ...
 showNotification('Notificacoes ativadas!', 'success');
 } else {
...
// ========================================
// APP UPDATE SYSTEM - Atualizacao In-App
// ========================================
...
async function checkAppVersion() {
 ...
 showUpdateDialog(data.version, data.changelog || 'Melhorias e correcoes.', data.downloadUrl, isRequired);
 }
 } catch (error) {
 console.log('Nao foi possivel verificar atualizacoes:', error);
 }
}
...
async function downloadUpdate(url) {
 if (!url) {
 showNotification('URL de download nao disponivel', 'error');
 return;
 }
 ...
 showNotification('Download iniciado! Apos baixar, instale o APK.', 'success');
 closeUpdateDialog();
 } catch (error) {
 console.error('Erro ao baixar atualizacao:', error);
...
async function forceAppUpdate() {
 ...
 // 5. Recarregar a pagina com cache busting
 setTimeout(() => {
...
async function refreshTokenInBackground() {
 ...
 const data = await response.json();
 if (data.ok) {
 // Atualizar token e usuario com dados novos
 state.token = data.token;
 state.user = data.user;
 localStorage.setItem('token', data.token);
 localStorage.setItem('user', JSON.stringify(data.user));
 
 // Atualizar permissoes com as novas roles
 setupPermissions();
 ...
 console.log('Nao foi possivel renovar token em background:', error);
 }
}
...
document.addEventListener('DOMContentLoaded', async () => {
 ...
 // Tentativa de retomar sessao salva (token + user)
...
function setupNavigation() {
 ...
 console.log('Navegacao configurada');
}
...
function navigateTo(view) {
 ...
 item.classList.remove('active');
 // Se e uma view principal, marcar diretamente
 if (mainMobileViews.includes(view) && item.getAttribute('data-view') === view) {
 item.classList.add('active');
 }
 });
 // Se nao e uma view principal, marcar "Mais" como ativo
 ...
 console.error('View nao encontrada para:', view);
 }
...
async function showAppWithWelcome() {
 ...
 // Pegar nome do usuario e empresa
 ...
 // Aguardar animacao (3.5 segundos) e depois mostrar o app
 setTimeout(() => {
...
async function showApp() {
 ...
 // Busca operador especifico por username
 ...
 // Setup permissions e navegacao (IMPORTANTE: refazer para garantir que funciona)
 setupPermissions();
 setupNavigation();
 ...
 // Load initial data
 await loadUsers(); // Carregar usuarios primeiro
 
 // Determinar view inicial baseado no tipo de tenant e permissoes
 ...
 if (isLavanderia) {
 // Lavanderias vao direto para o modulo de lavanderia
 ...
 // Inicializar busca rapida (Ctrl+K)
 initQuickSearch();
}
...
function setupPermissions() {
 ...
 // Se for lavanderia, so mostra modulos de lavanderia
 const isLavanderia = tenantType === 'lavanderia';
 
 // ========== SISTEMA DE PERMISSA•ES POR ABA ==========
 // Cada aba tem uma role especifica para VER e outra para EDITAR
 // Role 'admin'tem acesso total a tudo
 
 // Dashboard: dashboard (ver) - Lavanderia NAƒO tem dashboard separado, so o modulo lavanderia
 ...
 // Ordens de Servico: os (ver/criar), os_manage_all (gerenciar todas)
 // OS sempre visivel para granja, oculto para lavanderia
 const canSeeOS = !isLavanderia;
 
 // Almoxarifado: almoxarifado_view (ver), almoxarifado (editar) - so granja
 // Manutencao (os_manage_all) pode VER o almoxarifado para consultar pecas
 ...
 // Compras: compras_view (ver), compras (editar), compras_request (pode enviar pedidos) - so granja
 ...
 // Preventivas: preventivas_view (ver), preventivas (editar) - so granja
 ...
 // Checklists: checklist (ver), checklist_manage (editar) - so granja
 ...
 // Controle de Agua: agua (ver), agua_manage (editar) - so granja
 ...
 // Controle de Diesel: diesel (ver), diesel_manage (editar) - so granja
 ...
 // Gerador: gerador (ver), gerador_manage (editar) - so granja
 ...
 // Aditiva: aditiva_view (ver), aditiva (editar - so manutencao) - so granja
 ...
 // Lavanderia: so para tenant de lavanderia OU quem tem role lavanderia
 ...
 // Relatorios: relatorios (ver), relatorios_write (escrever - so manutencao) - so granja por enquanto
 ...
 // Salvar tipo de tenant e permissoes no state
 ...
 console.log('Permissoes configuradas. Roles:', roles, 'Pode editar diesel:', canEditDiesel, 'Pode editar gerador:', canEditGerador);
 
 // Atualizar navegacao mobile baseado nas permissoes
 setupMobileNavPermissions();
}

// Configura visibilidade dos itens da navegacao mobile baseado nas permissoes
function setupMobileNavPermissions() {
 ...
 // Mesmas regras de permissao do setupPermissions - considerando tenant_type
 // Para lavanderia: nao tem dashboard separado, so o modulo lavanderia
 const canSeeDashboard = !isLavanderia && (isAdmin || roles.includes('dashboard') || roles.includes('os_manage_all') || roles.includes('os_view_all'));
 const canSeeOS = !isLavanderia;
 const canSeeWater = !isLavanderia && (isAdmin || roles.includes('agua') || roles.includes('agua_manage') || roles.includes('os_manage_all'));
 const canSeeDiesel = !isLavanderia && (isAdmin || roles.includes('diesel') || roles.includes('diesel_manage') || roles.includes('os_manage_all'));
 ...
 // Itens da barra de navegacao mobile principal
 ...
 // Permissao para almoxarifado mobile
...
function setDashboardFilter(filter) {
 ...
 // Se escolheu mes especifico, forca monthly
 ...
 monthly: { period: 'mes', period2: 'do mes', productivity: '(Este Mes)'}
 };
...
function setDashboardMonth(monthValue) {
 ...
 if (monthValue) {
 // Forca filtro mensal quando seleciona mes especifico
 ...
 // Atualiza label com nome do mes
...
function initMonthSelector() {
 ...
 select.innerHTML = '<option value="">Mes Atual</option>'+ 
...
function getDateRange() {
 ...
 // Se tem mes especifico selecionado
 ...
 endDate.setDate(startDate.getDate() + 5); // Ate sabado
...
function updateDashboardStats() {
 ...
 const periodLabels = { daily: 'hoje', weekly: 'semana', monthly: 'mes'};
...
function renderChecklistTimeline(executions) {
 ...
 <div class="timeline-item-time">${time} â€¢ ${isAuto ? 'Automatico': 'Manual'}</div>
 </div>
 </div>
 `;
 }).join('');
...
function updateDashboardSummary(filteredOrders, completed, total) {
 ...
 // Tecnico destaque (quem mais concluiu)
 ...
 // Tempo medio (placeholder - precisa de finished_at no backend)
...
async function loadDashboardStats() {
 ...
 // Fallback para metodo antigo se endpoint nao existir
...
function renderProductivityChart() {
 ...
 <p style="font-size: 14px; margin: 0;">Nenhuma tarefa concluida ainda</p>
 </div>
 `;
 return;
 }
 ...
 ${avgTime ? `<span title="Tempo medio">${avgTime}</span>` : ''}
 </div>
 </div>
 `;
 }).join('');
...
function renderProductivityChartLegacy(filteredOrders = state.orders) {
 ...
 // Contar OS por usuario (assigned_users) - todos os usuarios da equipe de manutencao
 const userStats = {};
 ...
 // Usar o nome para exibicao mas username como chave
 ...
 `;
 }).join('') : '<p style="color: var(--text-secondary); padding: 20px;">Nenhuma OS concluida ainda</p>';
...
function filterOSByStatus(status) {
 // Se clicar no mesmo filtro que ja esta ativo, desativa
...
function renderOrdersTable() {
 ...
 // Sem filtro - mostrar apenas ativas (comportamento padrao)
 filteredOrders = state.orders.filter(o => o.status !== 'completed');
 }
 
 if (filteredOrders.length === 0) {
 const emptyMessage = state.osStatusFilter 
 ? `Nenhuma OS ${state.osStatusFilter === 'urgent'? 'urgente': state.osStatusFilter === 'completed'? 'concluida no mes': ''} encontrada`
...
function showOSDetail(orderId) {
 ...
 document.getElementById('detail-os-description').textContent = order.description || 'Sem observacoes';
 ...
 // Horarios de inicio, fim e tempo total
 ...
 // Calcular tempo total - usar worked_minutes se disponivel
 if (order.worked_minutes && order.worked_minutes > 0) {
 // Usar worked_minutes do backend (ja descontado o tempo de descanso)
 ...
 // Fallback: calcular diferenca bruta
 ...
 assignedContainer.innerHTML = '<span style="color: var(--text-secondary);">Nenhum tecnico atribuido</span>';
 }
 
 // Checkboxes para editar atribuicoes (se for o criador ou admin)
 ...
 // Mostrar secao de datas retroativas (so para manutencao/admin e quando nao concluida)
 ...
 // Checkboxes para editar atribuicoes
 // APENAS Manutencao/admin pode editar tecnicos (usuario comum nao pode mais)
 const canEditAssignments = canManageAll;
 
 if (canEditAssignments) {
 const assignedIds = order.assigned_users ? order.assigned_users.map(u => u.id) : [];
 const labelText = order.status === 'completed'? 'Editar tecnicos (OS concluida):': 'Alterar atribuicoes:';
 ...
 // Botao salvar - manutencao pode salvar mesmo em OS concluida
 if (canEditAssignments) {
 ...
 // Somente manutencao pode concluir
 if (canManageAll) {
 ...
 // Se estiver pausada, mostrar botao de retomar
 ...
 // Botao excluir - criador pode excluir sua OS, manutencao pode excluir qualquer
...
async function updateOSAssignments(orderId) {
 ...
 showNotification(`Resposta invalida do servidor: ${text.substring(0, 100)}`, 'error');
 return;
 }
 if (data.ok) {
 await loadOrders();
...
async function loadHistoryInOS() {
 ...
 tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; color: var(--text-secondary);">Nenhuma OS concluida</td></tr>';
 return;
 }
 ...
 // Calcular tempo total - usar worked_minutes se disponivel (ja com descanso descontado)
 let tempoTotal = '-';
 ...
 // Fallback: calcular diferenca bruta (sem descanso descontado)
 const diffMs = finishedAt - startedAt;
 const diffHours = Math.floor(diffMs / (1000 * 60 * 60));
 const diffMinutes = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60));
...
function toggleOSView(view) {
 ...
 loadHistoryInOS(); // Carregar historico ao mostrar
 }
}
...
// Pausar OS (para almoco, fim do dia, etc)
...
async function completeOrder(orderId) {
 // Verificar se alguem foi atribuido A  OS antes de concluir
 const order = state.orders.find(o => o.id === orderId);
 if (order) {
 const hasAssigned = order.assigned_users && order.assigned_users.length > 0;
 
 // Verificar tambem os checkboxes no modal (caso esteja aberto)
 const assignedUsernames = [];
 ['declie', 'eduardo', 'vanderlei', 'alissom'].forEach(username => {
 ...
 console.error('Erro ao atribuir usuarios:', e);
 }
 }
 }
 ...
 // Converter datetime-local para ISO com timezone de Brasilia (-03:00)
 // datetime-local retorna "2026-01-12T08:01" sem timezone
 // Adicionamos :00-03:00 para indicar que e horario de Brasilia
 const toISOLocal = (dtValue) => {
 if (!dtValue) return null;
 // Adicionar segundos e timezone de Brasilia
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('OS concluida com sucesso!', 'success');
 await loadOrders();
 closeModal('modal-os-detail');
 } else {
...
// Funcao especifica para fechar OS com datas retroativas
async function closeOrderWithRetroactiveDates() {
 ...
 // Buscar a OS pelo titulo ou iterar orders
 ...
 if (!order) {
 showNotification('Erro: OS nao encontrada. Feche e abra novamente.', 'error');
 return;
 }
 ...
 showNotification('Preencha pelo menos a Data/Hora de Inicio ou Conclusao!', 'error');
 return;
 }
 
 // Validar que a data de fim e posterior ao inicio
 if (startedAtCustom && finishedAtCustom) {
 const start = new Date(startedAtCustom);
 const end = new Date(finishedAtCustom);
 if (end <= start) {
 showNotification('A data de conclusao deve ser posterior A  data de inicio!', 'error');
 return;
 }
 }
 
 // Verificar se tem tecnico atribuido
 ...
 // Vamos garantir que seja interpretado como horario local de Brasilia (UTC-3)
 const toISOWithTimezone = (dateTimeLocalValue) => {
 if (!dateTimeLocalValue) return null;
 // dateTimeLocalValue vem como "2026-01-12T08:01"
 // Adicionar timezone de Brasilia (-03:00) para garantir interpretacao correta
 // Isso faz o backend entender que 08:01 e horario de Brasilia, nao UTC
...
async function deleteOrder(orderId) {
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('OS excluida com sucesso!', 'success');
 closeModal('modal-os-detail');
 await loadOrders();
 } else {
...
function showCreateOS() {
 ...
 // Mostrar secao de atribuicao apenas para manutencao
...
async function createOrderFromForm(event) {
 ...
 if (!title) {
 showNotification('Titulo e obrigatorio!', 'error');
 return;
 }
 ...
 // Buscar IDs dos usuarios
 const assignedUserIds = [];
 if (assignedUsernames.length > 0) {
 // Buscar users do state ou fazer requisicao
 await loadUsers();
 state.users.forEach(user => {
 if (assignedUsernames.includes(user.username.toLowerCase())) {
 assignedUserIds.push(user.id);
...
async function loadUsers() {
 if (state.users.length > 0) return; // Ja carregado
...
async function backupOrdersToTXT() {
 ...
 let content = `BACKUP ORDENS DE SERVIA‡O - ${timestamp}\n`;
 content += '='.repeat(100) + '\n\n';
 ...
 const statusLabels = { open: 'Aberta', in_progress: 'Em Andamento', completed: 'Concluida', cancelled: 'Cancelada'};
 const priorityLabels = { high: 'Alta', medium: 'Media', low: 'Baixa'};
 
 content += `OS #${order.id}\n`;
 content += `Titulo: ${order.title}\n`;
 content += `Status: ${statusLabels[order.status] || order.status}\n`;
 content += `Urgencia: ${priorityLabels[order.priority] || order.priority}\n`;
 content += `Local/Setor: ${order.sector || 'N/A'}\n`;
 content += `Solicitante: ${order.requested_by_name || 'N/A'}\n`;
 content += `Comentario: ${order.progress_note || order.description || 'N/A'}\n`;
 
 if (order.assigned_users && order.assigned_users.length > 0) {
 const techs = order.assigned_users.map(u => u.username).join(', ');
 content += `Tecnicos: ${techs}\n`;
 }
 ...
 content += `Concluida em: ${new Date(order.finished_at).toLocaleString('pt-BR')}\n`;
 }
 
 if (order.description) {
 content += `Descricao: ${order.description}\n`;
 }
...
// History
// NOTA: loadInventory() real esta mais abaixo no arquivo
...
// NOTA: Funcoes loadPurchases() e loadPreventives() reais estao mais abaixo no arquivo
...
async function loadReports() {
 ...
 // Calcular tempo - usar worked_minutes se disponivel
 if (order.worked_minutes && order.worked_minutes > 0) {
 // worked_minutes ja esta em minutos, converter para ms
 ...
 '<p style="color: var(--text-secondary); padding: 20px;">Nenhum dado disponivel</p>';
 ...
 <div class="productivity-bar">
 <div class="productivity-name">Media</div>
 <div class="productivity-bar-container">
...
function showNotification(message, type = 'info', duration = 5000, playSound = true) {
 ...
 try {
 // Som de notificacao estilo Discord (frequencia curta)
 ...
 // Adicionar ao inicio (notificacoes mais recentes em cima)
...
async function loadInventory() {
 ...
 state.inventory = cached;
 renderInventoryTable();
 showNotification('Sem conexao - usando dados salvos', 'warning');
 } else {
...
function renderInventoryMobile(items, categoryLabels) {
 ...
 mobileContainer.style.display = 'none'; // Escondido por padrao (CSS mobile mostra)
...
// Estado do modulo de almoxarifado V2
const almox2State = {
 currentTab: 'estoque',
 movements: [],
 pendingLoans: [],
...
function handleAlmoxSearch(input) {
 ...
 // Filtrar itens que contem a query
 ...
 const statusClass = status === 'Critico'? 'critico': status === 'Baixo'? 'baixo': '';
...
async function loadAlmoxMovements() {
 ...
 showNotification('Erro ao carregar movimentacoes', 'error');
 }
}
...
// Atualizar movimentacoes
function refreshMovements() {
 loadAlmoxMovements();
 showNotification('Movimentacoes atualizadas', 'success');
}
...
async function loadPendingLoans() {
 ...
 showNotification('Erro ao carregar emprestimos', 'error');
 }
}

// Renderizar emprestimos pendentes
function renderPendingLoans() {
 ...
 <p>Nenhum emprestimo pendente</p>
 </div>
 `;
 return;
 }
 ...
 const isOverdue = daysDiff > 7; // Mais de 7 dias e atrasado
...
// Atualizar estatisticas de ferramentas
function updateToolsStats() {
 ...
 // Ferramentas disponiveis = total de ferramentas que sao retornaveis
...
// Modal de confirmacao de devolucao - Design Premium
function showReturnConfirmation(movementId) {
 ...
 <h3 style="margin: 0; font-size: 18px; font-weight: 700; color: #fff;">Confirmar Devolucao</h3>
 </div>
 ...
 <div style="font-size: 11px; color: rgba(255,255,255,0.5); text-transform: uppercase; margin-bottom: 4px;">Responsavel</div>
...
// Registrar devolucao (agora chama o modal de confirmacao)
function registerReturn(movementId) {
 showReturnConfirmation(movementId);
}

// Executar a devolucao apos confirmacao
async function executeReturn(movementId) {
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('Devolucao registrada com sucesso!', 'success');
 loadPendingLoans();
 loadInventory();
 } else {
 showNotification('Erro ao registrar devolucao: '+ (data.error || 'Erro'), 'error');
 }
 } catch (error) {
 console.error('Erro ao registrar devolucao:', error);
 showNotification('Erro ao registrar devolucao', 'error');
 }
}
...
function showQuickWithdrawal() {
  ...
  const modalHtml = `
    <div id="modal-quick-withdrawal" class="modal-overlay active" onclick="if(event.target === this) closeModal('modal-quick-withdrawal')" style="backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); background: linear-gradient(135deg, rgba(0,0,0,0.85), rgba(20,10,40,0.9));">
      <div class="modal" style="
        ...
        border-radius: 24px;
        box-shadow: 0 25px 80px rgba(0, 0, 0, 0.6), 0 0 60px rgba(236, 72, 153, 0.15), inset 0 1px 0 rgba(255,255,255,0.05);
      ...
      ">
        <!-- Glow Effect Top -->
        <div style="position: absolute; top: -80px; left: 50%; transform: translateX(-50%); width: 300px; height: 160px; background: radial-gradient(ellipse, rgba(236, 72, 153, 0.25) 0%, transparent 70%); pointer-events: none;"></div>
        ...
        ">
          <div style="display: flex; align-items: center; gap: 16px;">
            ...
            ">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#f472b6" stroke-width="2.5" stroke-linecap="round">
            ...
            <div>
              <h3 style="margin: 0; font-size: 22px; font-weight: 700; color: #fff; letter-spacing: -0.3px;">Registrar Retirada</h3>
              <p style="margin: 5px 0 0; font-size: 13px; color: #8b8b9e;">Adicione itens e registre a saída do almoxarifado</p>
            </div>
          </div>
          <button type="button" onclick="closeModal('modal-quick-withdrawal')" style="
            width: 40px;
            height: 40px;
            background: rgba(255,255,255,0.05);
            ...
            justify-content: center;
            transition: all 0.2s;
          " onmouseover="this.style.background='rgba(239,68,68,0.2)';this.style.borderColor='rgba(239,68,68,0.3)';this.style.color='#ef4444'" onmouseout="this.style.background='rgba(255,255,255,0.05)';this.style.borderColor='rgba(255,255,255,0.1)';this.style.color='#888'">×</button>
        </div>
        
        <!-- Content Area -->
        <div style="padding: 24px 32px 28px; overflow-y: auto; flex: 1;">
          ...
          <div style="margin-bottom: 20px;">
            <label style="display: block; font-size: 11px; font-weight: 600; color: #f472b6; text-transform: uppercase; letter-spacing: 1.2px; margin-bottom: 10px;">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display: inline; vertical-align: middle; margin-right: 6px;"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              Buscar Item
            </label>
            <div style="position: relative;">
              <input type="text" id="withdrawal-search" placeholder="Digite o nome ou SKU do item..." autocomplete="off" style="
                width: 100%; 
                ...
                transition: all 0.2s;
                outline: none;
              " onfocus="this.style.borderColor='rgba(236,72,153,0.5)';this.style.boxShadow='0 0 24px rgba(236,72,153,0.15)'" onblur="this.style.borderColor='rgba(236,72,153,0.25)';this.style.boxShadow='none'" oninput="filterWithdrawalItems(this.value)">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f472b6" stroke-width="2" style="position: absolute; left: 16px; top: 50%; transform: translateY(-50%); opacity: 0.7;"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <div id="withdrawal-suggestions" style="display: none; position: absolute; top: 100%; left: 0; right: 0; background: rgba(20,20,30,0.98); backdrop-filter: blur(12px); border: 1px solid rgba(236,72,153,0.3); border-radius: 14px; margin-top: 6px; max-height: 220px; overflow-y: auto; z-index: 100; box-shadow: 0 12px 40px rgba(0,0,0,0.4);"></div>
            </div>
          </div>
          ...
          <div style="margin-bottom: 22px;">
            <label style="display: block; font-size: 11px; font-weight: 600; color: #f472b6; text-transform: uppercase; letter-spacing: 1.2px; margin-bottom: 10px;">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display: inline; vertical-align: middle; margin-right: 6px;"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
              Itens Selecionados
            </label>
            <div id="withdrawal-selected-items" style="min-height: 80px; background: linear-gradient(135deg, rgba(236,72,153,0.05), rgba(168,85,247,0.03)); border: 1px dashed rgba(236,72,153,0.25); border-radius: 14px; padding: 16px; display: flex; align-items: center; justify-content: center;">
              <p style="color: rgba(255,255,255,0.35); text-align: center; margin: 0; font-size: 13px;">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="display: block; margin: 0 auto 8px; opacity: 0.4;"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
                Busque e adicione itens acima
              </p>
            </div>
          </div>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px;">
            ...
            <div>
              <label style="display: block; font-size: 11px; font-weight: 600; color: #f472b6; text-transform: uppercase; letter-spacing: 1.2px; margin-bottom: 10px;">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display: inline; vertical-align: middle; margin-right: 6px;"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                Setor / Destino
              </label>
              <select id="withdrawal-sector" style="
                width: 100%; 
                padding: 14px 16px; 
                background: rgba(255,255,255,0.03);
                border: 1px solid rgba(255,255,255,0.1);
                ...
                cursor: pointer;
                appearance: none;
                background-image: url('data:image/svg+xml;charset=UTF-8,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%2212%22 height=%2212%22 viewBox=%220 0 24 24%22 fill=%22none%22 stroke=%22%23f472b6%22 stroke-width=%222%22%3E%3Cpolyline points=%226 9 12 15 18 9%22/%3E%3C/svg%3E');
              ...
              ">
                ...
                <option value="Aviario 1">Aviário 1</option>
                <option value="Aviario 2">Aviário 2</option>
                <option value="Aviario 3">Aviário 3</option>
                <option value="Aviario 4">Aviário 4</option>
                <option value="Recria">Recria</option>
                <option value="Fabrica de Racao">Fábrica de Ração</option>
          ...
          <div style="margin-bottom: 24px;">
            ...
            <textarea id="withdrawal-notes" rows="2" placeholder="Anotações adicionais, motivo da retirada..." style="
              width: 100%; 
              padding: 14px 16px; 
              background: rgba(255,255,255,0.03);
              border: 1px solid rgba(255,255,255,0.1);
              ...
              transition: all 0.2s;
              outline: none;
            " onfocus="this.style.borderColor='rgba(236,72,153,0.5)';this.style.boxShadow='0 0 20px rgba(236,72,153,0.12)'" onblur="this.style.borderColor='rgba(255,255,255,0.1)';this.style.boxShadow='none'"></textarea>
          </div>
          ...
          <div style="display: flex; gap: 14px;">
            <button type="button" onclick="closeModal('modal-quick-withdrawal')" style="
              ...
              font-size: 14px;
              transition: all 0.2s;
            " onmouseover="this.style.background='rgba(255,255,255,0.08)';this.style.color='#fff'" onmouseout="this.style.background='rgba(255,255,255,0.04)';this.style.color='#aaa'">
              Cancelar
            </button>
            <button type="button" onclick="submitMultipleWithdrawal()" style="
              flex: 1.6; 
              padding: 16px 24px; 
              ...
              transition: all 0.2s;
            " onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 12px 28px rgba(236,72,153,0.45)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='0 8px 24px rgba(236,72,153,0.35)'">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/></svg>
              Confirmar Retirada
            </button>
          </div>
        </div>
...
function filterWithdrawalItems(query) {
  ...
  suggestionsDiv.innerHTML = items.map(item => `
    <div onclick="addWithdrawalItem('${item.id}')" style="
      padding: 14px 18px; 
      cursor: pointer; 
      ...
      align-items: center;
      gap: 14px;
    " onmouseover="this.style.background='linear-gradient(90deg, rgba(236,72,153,0.15), rgba(168,85,247,0.1))'" onmouseout="this.style.background='transparent'">
      <div style="width: 40px; height: 40px; background: linear-gradient(135deg, rgba(236,72,153,0.2), rgba(168,85,247,0.15)); border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#f472b6" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
      </div>
      <div style="flex: 1; min-width: 0;">
        <div style="font-weight: 600; color: #fff; font-size: 14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${escapeHtml(item.name)}</div>
        <div style="font-size: 12px; color: rgba(255,255,255,0.5); display: flex; gap: 8px; margin-top: 2px;">
          <span style="color: #4ade80;">${item.quantity} ${item.unit || 'un'}</span>
          <span>•</span>
          <span>${item.category || 'Sem categoria'}</span>
        </div>
      </div>
      <div style="background: rgba(34,197,94,0.15); color: #4ade80; padding: 4px 10px; border-radius: 6px; font-size: 11px; font-weight: 600;">+ Adicionar</div>
    </div>
  `).join('');
  ...
  container.innerHTML = withdrawalItems.map((item, idx) => `
    ...
    ">
      <div style="
        ...
        flex-shrink: 0;
      ">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f472b6" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
      </div>
      <div style="flex: 1; min-width: 0;">
        <div style="font-weight: 600; color: #fff; font-size: 14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${escapeHtml(item.name)}</div>
        <div style="font-size: 11px; color: rgba(255,255,255,0.45); margin-top: 2px;">Disponível: ${item.maxQty} ${item.unit}</div>
      </div>
      <div style="display: flex; align-items: center; gap: 6px; background: rgba(0,0,0,0.2); padding: 6px 8px; border-radius: 10px;">
        <button type="button" onclick="changeWithdrawalQty(${idx}, -1)" style="
          width: 30px; 
          height: 30px; 
          border-radius: 8px; 
          ...
          justify-content: center;
          transition: all 0.15s;
        " onmouseover="this.style.background='rgba(236,72,153,0.2)';this.style.borderColor='rgba(236,72,153,0.4)'" onmouseout="this.style.background='rgba(255,255,255,0.05)';this.style.borderColor='rgba(255,255,255,0.15)'">−</button>
        <span style="color: #fff; font-weight: 700; min-width: 36px; text-align: center; font-size: 16px;">${item.qty}</span>
        <button type="button" onclick="changeWithdrawalQty(${idx}, 1)" style="
          width: 30px; 
          height: 30px; 
          border-radius: 8px; 
          ...
          justify-content: center;
          transition: all 0.15s;
        " onmouseover="this.style.background='rgba(34,197,94,0.2)';this.style.borderColor='rgba(34,197,94,0.4)'" onmouseout="this.style.background='rgba(255,255,255,0.05)';this.style.borderColor='rgba(255,255,255,0.15)'">+</button>
      </div>
      <button type="button" onclick="removeWithdrawalItem(${idx})" style="
        width: 34px; 
        height: 34px; 
        border-radius: 10px; 
        ...
        justify-content: center;
        transition: all 0.15s;
      " onmouseover="this.style.background='rgba(239,68,68,0.25)';this.style.borderColor='rgba(239,68,68,0.5)'" onmouseout="this.style.background='rgba(239,68,68,0.1)';this.style.borderColor='rgba(239,68,68,0.3)'">
...
function showQuickEntry() {
 ...
 <textarea name="notes" rows="2" placeholder="Anotacoes sobre a entrada..." style="width: 100%; padding: 14px 16px; background: rgba(0,0,0,0.25); border: 1px solid rgba(34,197,94,0.25); border-radius: 12px; color: #fff; font-size: 14px; resize: none; transition: all 0.2s ease; line-height: 1.5;" onfocus="this.style.borderColor='rgba(34,197,94,0.6)'; this.style.boxShadow='0 0 0 3px rgba(34,197,94,0.15)';" onblur="this.style.borderColor='rgba(34,197,94,0.25)'; this.style.boxShadow='none';"></textarea>
 </div>
...
function renderAlmoxReports(stats) {
 ...
 <div class="almox2-top-rank">${idx + 1}</div>
 <div class="almox2-top-info">
 <div class="almox2-top-name">${escapeHtml(sector.person_sector || 'Nao informado')}</div>
 </div>
 ...
 // Renderizar graficos se Chart.js disponivel
...
function renderAlmoxCharts(stats) {
 ...
 type: 'bar',
 data: {
 labels: stats.chart_labels || ['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sab', 'Dom'],
 datasets: [
 {
 label: 'Entradas',
 ...
 // Grafico de emprestimos: Devolvidos vs Pendentes
 ...
 // Grafico por Tipo de Uso (Consumo, Emprestimo, Manutencao)
...
function showAlmoxReportModal() {
 ...
 <h3 style="margin: 0; color: #fff;">Relatorio do Almoxarifado</h3>
...
// Exportar Relatorio HTML Premium
...
<head>
 <meta charset="UTF-8">
 <meta name="viewport" content="width=device-width, initial-scale=1.0">
 <title>Relatorio Almoxarifado - ${dateStr}</title>
 <style>
 * { margin: 0; padding: 0; box-sizing: border-box; }
...
<body>
 ...
 <h1>Relatorio de Almoxarifado</h1>
 <p class="subtitle">Controle Completo de Estoque e Materiais</p>
 <div class="date-badge">Gerado em ${dateStr} A s ${timeStr}</div>
 </div>
 ...
 <span class="low-stock-title">Atencao: Itens em Estoque Baixo (${lowStock.length})</span>
 </div>
 <table class="items-table">
 <thead>
 <tr>
 ...
 <div class="footer">
 <p>Relatorio gerado pelo sistema <strong>ICARUS</strong></p>
 <p style="margin-top: 8px;">Sistema de Gestao Inteligente de Manutencao</p>
 ...
 <div style="font-size: 11px; color: rgba(255,255,255,0.5);">Sistema Inteligente de Gestao</div>
 </div>
 </div>
 </div>
 ...
 title: 'Relatorio Almoxarifado - Granja Vitta',
 ...
 // Renderizar diretamente na pagina (funciona em APK, mobile e desktop)
 showReportInPage(html, 'Relatorio Almoxarifado', 'Relatorio do Almoxarifado gerado!', almoxarifadoReportData);
...
function showItemDetail(itemId) {
 ...
 <div style="font-size: 12px; color: #64748b;">Nenhuma especificacao cadastrada</div>
 <div style="font-size: 11px; color: #475569; margin-top: 4px;">Clique em "Editar Item" para adicionar detalhes tecnicos</div>
 </div>
 </div>
 `}
...
function showEditItemModal(itemId) {
 ...
 // TODO: Criar modal de edicao dedicado
 showNotification('Funcao de edicao em desenvolvimento', 'info');
}
...
async function adjustStock(itemId, delta) {
 ...
 if (newQuantity < 0) {
 showNotification('Quantidade nao pode ser negativa', 'error');
 return;
 }
 if (item.max_stock && newQuantity > item.max_stock) {
 showNotification('Acima do estoque maximo definido', 'warning');
 return;
 }
...
async function deleteItem(itemId) {
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('[ALMOXARIFADO] Item excluido', 'success');
 await loadInventory();
 // await backupInventoryToTXT(); // Desabilitado temporariamente
 } else {
...
async function backupInventoryToTXT() {
 ...
 content += `Estoque Minimo: ${item.min_stock || 0}\n`;
 content += `Estoque Maximo: ${item.max_stock || '-'}\n`;
 content += `Categoria: ${item.category || '-'}\n`;
 content += `Marca: ${item.brand || '-'}\n`;
 content += `Localizacao: ${item.location || 'N/A'}\n`;
 content += `Ultima Atualizacao: ${item.updated_at ? new Date(item.updated_at).toLocaleString('pt-BR') : '-'}\n`;
 content += '-'.repeat(80) + '\n\n';
 });
...
// ========================================
// ALMOXARIFADO V2 - EMPRESTIMO DE ITENS RETORNAVEIS
// ========================================

// Modal de emprestimo de item retornavel
function showLoanItemModal(itemId) {
 const item = state.inventory.find(i => i.id === itemId);
 if (!item) return;
 
 if (!item.is_returnable) {
 showNotification('Este item nao e retornavel. Use saida normal.', 'warning');
 return;
 }
 ...
 <p style="margin: 4px 0 0 0; font-size: 12px; color: #64748b;">${escapeHtml(item.name)} â€¢ Disponivel: ${item.quantity} ${item.unit}</p>
 </div>
 ...
 <div>
 <label style="display: block; font-size: 12px; color: #94a3b8; margin-bottom: 6px;">Quem esta pegando? *</label>
 ...
 <div>
 <label style="display: block; font-size: 12px; color: #94a3b8; margin-bottom: 6px;">Observacao (opcional)</label>
...
// Modal de emprestimos pendentes de um item
async function showItemLoans(itemId) {
 ...
 const returnedLoans = loans.filter(l => l.returned_at).slice(0, 10); // Ultimos 10
 ...
 <div>
 <h3 style="margin: 0; font-size: 16px; color: #fff;">Emprestimos: ${escapeHtml(item.name)}</h3>
 ...
 <div style="font-size: 11px; color: #64748b;">Pegou em ${new Date(loan.borrowed_at).toLocaleDateString('pt-BR')} A s ${new Date(loan.borrowed_at).toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'})}</div>
 ...
 <p style="margin: 0;">Nenhum emprestimo pendente</p>
 </div>
 `}
...
async function toggleItemReturnable(itemId, isReturnable) {
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification(isReturnable ? 'Item marcado como retornavel': 'Item marcado como nao-retornavel', 'success');
 await loadInventory();
 } else {
...
function updateFinancialDashboard() {
 ...
 // Economia vs orcamento (exemplo: orcamento mensal de R$ 10.000)
...
function renderPurchasesTable() {
 ...
 tbody.innerHTML = '<tr><td colspan="8" style="text-align:center">Nenhuma requisicao cadastrada</td></tr>';
 return;
 }
 ...
 ? `<img src="${purchase.photo_url}" class="purchase-photo-thumb" onclick="event.stopPropagation(); showPurchasePhoto('${purchase.id}')" title="Clique para ampliar" alt="Foto da peca">`
...
function showAdvancePurchaseModal(purchaseId) {
 ...
 pedido: { next: 'chegando', label: 'Em Transito'},
 ...
 <textarea name="notes" placeholder="Adicione informacoes relevantes..."></textarea>
 </div>
 
 <div class="modal-actions">
 <button type="button" class="btn-small btn-cancel" onclick="closeModal('modal-advance-purchase')">Cancelar</button>
 <button type="submit" class="btn-small btn-primary">Confirmar Avanco</button>
 </div>
 </form>
 </div>
 </div>
...
// Funcao para comprimir imagem antes de enviar
...
async function createPurchaseFromForm(event) {
 ...
 // Converter foto para base64 se existir (com compressao automatica)
 let photoUrl = null;
 const photoInput = document.getElementById('purchase-photo');
 if (photoInput && photoInput.files && photoInput.files[0]) {
 const file = photoInput.files[0];
 // Comprimir imagem automaticamente (max 800px, qualidade 70%)
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('[COMPRAS] Requisicao criada com sucesso', 'success');
 closeModal('modal-create-purchase');
 clearPurchasePhoto();
 await loadPurchases();
 } else {
 showNotification('Erro ao criar requisicao: '+ (data.error || 'Erro desconhecido'), 'error');
 }
 } catch (error) {
 console.error('Erro ao criar requisicao:', error);
 showNotification('Erro ao criar requisicao', 'error');
 }
}
...
async function advancePurchaseWithDetails(event, purchaseId) {
 ...
 console.error('Erro ao avancar:', error);
 showNotification('Erro ao avancar status', 'error');
 }
}

// Funcao para mostrar foto ampliada
...
async function deletePurchase(purchaseId) {
 if (!confirm('Tem certeza que deseja excluir esta requisicao?')) return;
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('[COMPRAS] Requisicao excluida', 'success');
 await loadPurchases();
 } else {
 showNotification('Erro ao excluir requisicao', 'error');
 }
 } catch (error) {
...
function renderPreventivesTable() {
 ...
 // Ordenar: vencidas primeiro, depois por data mais proxima
 ...
 // Verificar alertas de vencimento proximo
...
async function markPreventiveDone(id) {
 if (!confirm('Marcar esta preventiva como concluida? A proxima data sera recalculada automaticamente.')) return;
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('Preventiva concluida! Proxima data atualizada.', 'success');
 await loadPreventives();
 } else {
...
async function completePreventive(preventiveId) {
 ...
 if (!confirm(`Marcar "${prev.equipment_name}" como concluida?`)) return;
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('[PREVENTIVA] Concluida! Proxima data agendada.', 'success');
 await loadPreventives();
 } else {
...
async function deletePreventive(preventiveId) {
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('[PREVENTIVA] Excluida', 'success');
 await loadPreventives();
 } else {
...
function loadConfigurations() {
 ...
 // Limpar credenciais salvas (mas manter a sessao atual)
...
async function verifyWhatsApp() {
 ...
 if (!phone) {
 alert('Por favor, insira seu numero de WhatsApp');
 return;
 }
 ...
 alert('Numero invalido. Use o formato: (XX) XXXXX-XXXX');
 return;
 }
 ...
 showNotification('[WHATSAPP] Verificado! Voce recebera notificacoes.', 'success');
 } else {
...
function openWhatsAppSupport() {
 // Seu numero de WhatsApp para suporte
 const supportPhone = '5511999999999'; // ALTERAR PARA SEU NUMERO
 const message = encodeURIComponent('Ola! Preciso de ajuda com o sistema Icarus.');
 window.open(`https://wa.me/${supportPhone}?text=${message}`, '_blank');
}

function openDocumentation() {
 alert('Documentacao em desenvolvimento!\n\nEm breve teremos um guia completo do sistema.');
}
...
function showQuickSearch() {
 ...
 results.push({ type: 'Peca', title: i.name, id: i.sku, action: () => { modal.remove(); navigateTo('almoxarifado'); } });
 }
 });
...
function renderChecklists() {
 ...
 <p style="margin: 0; font-size: 13px;">Clique em "Novo Checklist" para comecar</p>
 </div>
 `;
 return;
 }
 ...
 // Verificar se automacao esta ativa
 ...
 const freqLabel = autoFreqDays === 1 ? 'Diario': autoFreqDays === 2 ? 'Dia S/N': autoFreqDays === 7 ? 'Semanal': `${autoFreqDays}d`;
 ...
 ${isAutoEnabled ? 'Automatico': 'Automatizar'}
 </button>
...
// Mostrar modal de automacao de checklist
function showChecklistAutomation(checklistId) {
 ...
 // Carregar tecnicos para o select
 ...
 <div style="flex: 1;">
 <h3 style="margin: 0; font-size: 20px; color: #fff; font-weight: 700;">Automacao de Checklist</h3>
 ...
 <div>
 <span style="font-size: 15px; font-weight: 600; color: #fff;">Execucao Automatica</span>
 <p style="margin: 2px 0 0 0; font-size: 12px; color: rgba(255,255,255,0.5);">Marca como concluido automaticamente</p>
 </div>
 </div>
 ...
 <button type="button" onclick="setAutoFreq(1)" class="freq-btn" data-freq="1" style="padding: 12px 8px; background: ${freqDays === 1 ? 'linear-gradient(135deg, #a855f7, #7c3aed)': 'rgba(255,255,255,0.05)'}; border: 1px solid ${freqDays === 1 ? 'transparent': 'rgba(255,255,255,0.15)'}; border-radius: 10px; color: #fff; font-size: 12px; font-weight: 600; cursor: pointer; text-align: center;">Diario</button>
 ...
 <div>
 <span style="font-size: 14px; font-weight: 600; color: #fff;">Criar OS Automatica</span>
 ...
 <label style="display: block; font-size: 12px; color: rgba(255,255,255,0.6); margin-bottom: 6px;">Titulo da OS</label>
 <input type="text" id="auto-os-title" value="${escapeHtml(autoOsTitle)}" placeholder="Titulo da OS" style="width: 100%; padding: 12px 14px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.12); border-radius: 10px; color: #fff; font-size: 14px;">
 </div>
 <div>
 <label style="display: block; font-size: 12px; color: rgba(255,255,255,0.6); margin-bottom: 6px;">Responsavel (quem executou)</label>
 ...
 <button onclick="saveChecklistAutomation('${checklistId}')" style="flex: 1; padding: 15px; background: linear-gradient(135deg, #a855f7, #7c3aed); border: none; border-radius: 12px; color: #fff; font-size: 14px; font-weight: 600; cursor: pointer; box-shadow: 0 4px 20px rgba(168, 85, 247, 0.4);">Salvar Configuracao</button>
 </div>
 </div>
 </div>
 </div>
...
// Salvar configuracao de automacao
async function saveChecklistAutomation(checklistId) {
 ...
 // Validar se escolheu responsavel quando criar OS
 if (enabled && autoCreateOS && !autoOsExecutor) {
 showNotification('Selecione um responsavel para a OS automatica', 'warning');
 return;
 }
 ...
 renderChecklists();
 showNotification(enabled ? `Automacao ativada para ${autoTime}!` : 'Automacao desativada', 'success');
 } else {
 showNotification(data.error || 'Erro ao salvar automacao', 'error');
 }
 } catch (error) {
 console.error('Erro ao salvar automacao:', error);
 showNotification('Erro ao salvar automacao', 'error');
 }
}
...
function openExecuteChecklist(checklistId) {
 ...
 // Mostrar opcao de criar OS apenas para manutencao
...
// Toggle secao de executor quando marca criar OS
...
// Carregar lista de tecnicos para o select
async function loadChecklistOSExecutors() {
 ...
 select.innerHTML = '<option value="">-- Selecione o tecnico --</option>'+
...
async function submitChecklistExecution() {
 ...
 showNotification('Selecione um tecnico para a OS', 'warning');
 return;
 }
 ...
 const data = await response.json();
 if (data.ok) {
 // Se marcou para criar OS, criar a OS tambem
 if (createOS && executorId) {
...
async function createOSFromChecklist(checklistId, executorId, notes) {
 ...
 // Montar descricao com os itens do checklist
 ...
 sector: checklist.sector || 'Manutencao',
 priority: 'medium',
 ...
 const data = await response.json();
 if (data.ok) {
 showNotification('Ordem de Servico criada!', 'success');
...
async function viewChecklistHistory(checklistId) {
 ...
 if (executions.length === 0) {
 container.innerHTML = '<p style="color: var(--text-secondary); padding: 20px; text-align: center;">Nenhuma execucao registrada</p>';
 } else {
 ...
 <strong>${escapeHtml(exec.executed_by_name) || 'Usuario'}</strong>
 ...
 showNotification('Erro ao carregar historico: '+ error.message, 'error');
 }
}
...
async function deleteChecklist(checklistId) {
 ...
 state.checklists = data.checklists || [];
 renderChecklists();
 showNotification('Checklist excluido!', 'success');
 } else {
...
// ========== CONTROLE DE AGUA - GRANJA VITTA ==========

// Estado do controle de agua
state.waterReadings = [];
state.waterPeriod = 'day';
state.waterStats = null;
state.waterChartType = 'consumo'; // 'consumo'ou 'temperatura'

// Buscar temperatura atual da internet (Granja Vitta - Aparecida de Goiania)
async function fetchCurrentTemperature() {
 try {
 // Coordenadas da Granja Vitta - Aparecida de Goiania, GO
 const lat = -16.8225;
 const lon = -49.2433;
 
 // API Open-Meteo (gratuita, sem chave necessaria)
 ...
 console.log('Nao foi possivel obter temperatura automatica:', error);
 }
 return null;
}

// Carregar controle de agua
async function loadWaterControl() {
 try {
 // Verificar permissao - Bruno e JoseWalter so visualizam
 ...
 // Buscar temperatura automatica da internet
 fetchCurrentTemperature();
 
 // Atualizar horario atual
 updateCurrentTime();
 setInterval(updateCurrentTime, 60000);
 }
 
 // Carregar dados EM PARALELO (mais rapido)
 await Promise.all([
 loadWaterReadings(),
 loadWaterStats()
 ]);
...
function renderWaterStats() {
 ...
 // Funcao para formatar data corretamente
 ...
 // SEMPRE usar data de HOJE para exibicao principal
 ...
 // Sempre mostra Pendente para hoje se nao tem leitura
 ...
 // Calcular consumo do periodo de trabalho (7h-16h = 9 horas) - so se ambos existem HOJE
 let consumoTrabalho = '--';
 let ltHoraTrabalho = '--';
 ...
 // Calcular consumo 24h do ULTIMO DIA COMPLETO (ontem)
 ...
 // Filtrar leituras pelo periodo selecionado
 ...
 // Comparativo - usar leituras do periodo selecionado
 ...
 // Atualizar card de Total das Caixas no Periodo
 ...
 else if (period === 'month') elPeriodoLabel.textContent = 'Este Mes';
 }
 ...
 // Mini charts - gerar a partir das leituras filtradas pelo periodo
...
// Renderizar grafico principal
function renderWaterChart() {
 // Se estiver no modo temperatura, renderizar o grafico de temperatura
 ...
 container.innerHTML = '<p style="color: var(--text-secondary); text-align: center; padding: 40px;">Registre leituras para ver o grafico</p>';
 return;
 }
 ...
 // Funcao para calcular o dia anterior
 ...
 // Calcular consumo diario a partir das leituras 7h
 // O consumo do DIA X e a diferenca: Leitura 7h do dia (X+1) - Leitura 7h do dia X
 // Entao atribuimos o consumo ao DIA ANTERIOR da leitura nova
 ...
 if (diff >= 0) {
 // Consumo e atribuido ao dia ANTERIOR (dia X, nao dia X+1)
 ...
 if (dates.length === 0) {
 container.innerHTML = '<p style="color: var(--text-secondary); text-align: center; padding: 40px;">Registre leituras de pelo menos 2 dias para ver o grafico</p>';
 return;
 }
...
// Alternar tipo de grafico de agua (consumo/temperatura)
...
// Renderizar grafico de temperatura - Mostra Minima (7h) e Maxima (16h) do dia
function renderTemperatureChart() {
 ...
 // Filtrar leituras que tem temperatura
 ...
 // Agrupar por data - pegar temperatura das 7h (minima) e 16h (maxima)
 var tempByDate = {};
 ...
 // Ponto e linha da 7h (minima - azul)
 if (data.temp7h !== null) {
 ...
 '<span style="font-size:11px;color:var(--text-secondary);">7h (Minima)</span>'+
 '</div>'+
 '<div style="display:flex;align-items:center;gap:6px;">'+
 '<div style="width:10px;height:10px;background:#ef4444;border-radius:50%;"></div>'+
 '<span style="font-size:11px;color:var(--text-secondary);">16h (Maxima)</span>'+
 '</div>'+
 '</div>'+
...
function renderWaterHistory() {
 ...
 // Funcao para formatar data corretamente (evitar timezone issues)
 ...
 // Funcao para calcular o dia seguinte
 ...
 // Ordenar por data DESC, horario DESC
 ...
 // Se 'all'ou nao definido: mostrar todas
 ...
 // Ou seja, so podemos mostrar consumo 24h se tivermos a leitura do dia SEGUINTE
 const today = new Date().toISOString().split('T')[0];
 
 // Criar mapa de datas unicas para cores alternadas
 ...
 const tankLabel = reading.tank_name === 'aviarios'? 'Aviarios': 'Recria';
 ...
 if (filterConsumption === '24h') {
 // Consumo 24h: so para leituras das 7h
 ...
 // Consumo 9h (7h A s 16h): so para leituras das 16h
...
async function deleteWaterReading(id) {
 ...
 renderWaterHistory();
 renderWaterChart();
 showNotification('Leitura excluida com sucesso!', 'success');
 } else {
...
function checkWaterAlerts() {
 ...
 // Verificar consumo anormal (mais que 2x a media)
 const alerts = [];
 ...
 alerts.push({
 tank: tank === 'aviarios'? 'Aviarios': 'Recria',
 message: `Consumo ${((lastConsumption / avg - 1) * 100).toFixed(0)}% acima da media!`,
...
async function saveWaterReading() {
 ...
 if (!date || !time) {
 showNotification('Preencha a data e horario', 'error');
 return;
 }
...
// Estado para relatorio de agua
state.waterReportMonth = null; // null = mes atual

// Exportar relatorio PDF - DESIGN PREMIUM
function exportWaterReportPDF(selectedMonth) {
 ...
 // CALCULO 24H: Consumo do Dia X = Leitura 7h do Dia X+1 - Leitura 7h do Dia X
 // Isso mede o consumo REAL de 24 horas
 function calculateConsumption(tank) {
 // Filtrar leituras do tanque especifico
 const tankReadings = sortedReadings.filter(r => r.tank_name === tank);
 
 if (tankReadings.length === 0) return { total: 0, avg: 0, days: 0, dailyData: {} };
 
 // Pegar apenas leituras das 7h (ou a primeira do dia se nao tiver 7h)
 ...
 // Substituir por leitura da manha
 morningReadings[dayKey] = { reading: r, dateObj: dateObj, isMorning: isMorning, time: timeMinutes };
 } else if (isMorning && morningReadings[dayKey].isMorning) {
 // Ambas sao manha, pegar a mais proxima das 7h
 ...
 dailyConsumption[currentDay] = 0; // Pode ser reset do hidrometro ou erro
 }
 }
 ...
 // Gerar opcoes de meses disponiveis
 ...
 '<td class="tank-'+ r.tank_name + '">'+ (r.tank_name === 'aviarios'? 'Aviarios': 'Recria') + '</td>'+
 ...
 '<title>Relatorio de Agua - Granja Vitta</title>'+
 ...
 '<div class="table-scroll"><table><thead><tr><th>Data</th><th>Dia</th><th>Horario</th><th>Caixa</th><th>Leitura</th><th>Registrado por</th><th>Obs</th></tr></thead><tbody>'+ tableRows + '</tbody></table></div>'+
 '</div>'+
 '<div class="footer">'+
 ...
 title: 'Relatorio de Agua - Granja Vitta',
 type: 'water-report',
 content: {
 period: periodStr,
 summary: {
 ...
 // Renderizar diretamente na pagina (funciona em APK, mobile e desktop)
 showReportInPage(htmlContent, 'Relatorio de Agua', 'Relatorio de Agua gerado!', waterReportData);
}

// Funcao utilitaria para mostrar relatorios na pagina atual (100% compativel com APK/WebView)
...
function showReportInPage(htmlContent, reportTitle, successMessage, reportData) {
 ...
 // Salvar dados estruturados para geracao de PDF no servidor
 ...
 // Botao Salvar PDF (usa print com opcao salvar)
 ...
 // Container do conteudo com scroll
...
// Funcao para baixar PDF - Navega diretamente para endpoint que retorna PDF real
async function downloadReportAsPDF() {
 ...
 // Garantir que a URL nao termina com /
 if (API_URL.endsWith('/')) {
 ...
 // Detectar tipo de relatorio pelo titulo atual
 ...
 } else if (currentReportTitle && currentReportTitle.toLowerCase().includes('agua')) {
 ...
 // Metodo 1: Usar Capacitor Browser plugin (recomendado)
 try {
 ...
 // Metodo 2: Usar App.openUrl (Capacitor App plugin)
 try {
 ...
 // Metodo 3: Usar Cordova InAppBrowser (se disponivel)
 try {
 ...
 // Fallback para relatorios sem endpoint especifico: usar print
 showNotification('Abrindo impressao... Selecione "Salvar como PDF"', 'info');
...
// Funcao auxiliar para carregar scripts dinamicamente
...
// Exportar relatorio Excel (CSV) - Formato oficial Granja Vitta
function exportWaterReportExcel() {
 ...
 // Funcao para formatar data corretamente no padrao DD/MM/YYYY
 ...
 var diasSemana = ['DOMINGO', 'SEGUNDA', 'TERA‡A', 'QUARTA', 'QUINTA', 'SEXTA', 'SABADO'];
 ...
 // Proximo dia para calcular consumo 24h
 var nextDate = dates[idx + 1];
 var nextData = nextDate ? dailyData[nextDate] : null;
 
 // RECRIA - Periodo de trabalho (7AM-4PM)
 ...
 // AVIARIOS - Periodo de trabalho (7AM-4PM)
 ...
 // Criar CSV com cabecalhos claros
 var headers = ['DATA', 'DIA/SEMANA', 'CAIXA', 'HORAS', 'ENTRADA (M3)', 'LT POR HORA', 'LT TOTAL', 'PERIODO'];
 
 var csvContent = headers.join(';') + '\n';
 rows.forEach(function(row) {
 // Formatar a data como texto puro para o Excel nao interpretar errado
 ...
 // Tambem gerar relatorio HTML interativo
 generateInteractiveReport(rows, dailyData, dates);
}

// Gerar relatorio HTML interativo e tecnologico
function generateInteractiveReport(rows, dailyData, dates) {
 // Calcular totais para os graficos
 ...
 chartDataTrabalho.push({ date: formattedDate, tank: 'Aviarios', value: consumo });
 ...
 chartData24h.push({ date: formattedDate, tank: 'Aviarios', value: consumo });
...
<head>
 <meta charset="UTF-8">
 <meta name="viewport" content="width=device-width, initial-scale=1.0">
 <title>Relatorio de Controle de Agua - Granja Vitta</title>
...
<body>
 ...
 if (d.tank === 'Aviarios') grouped[d.date].aviarios = d.value;
 });
 return grouped;
 }
 ...
 // Grafico de barras horizontais por caixa
...
// Obter label do periodo
function getPeriodLabel() {
 switch (state.waterPeriod) {
 case 'day': return 'Hoje';
 case 'week': return 'Ultima Semana';
 case 'month': return 'Ultimo Mes';
 default: return 'Periodo';
 }
}

// ========== FIM CONTROLE DE AGUA ==========

// ========== CONTROLE DE DIESEL ==========

// Estado adicional para mes selecionado no diesel
if (typeof state.dieselSelectedMonth === 'undefined') {
 state.dieselSelectedMonth = null; // null = mes atual, ou 'YYYY-MM'para mes especifico
}

// Funcao auxiliar para obter datas do periodo (diesel)
function getDieselPeriodDates() {
 var now = new Date();
 var year, month, day, endDate, startDate;
 
 // Se um mes especifico foi selecionado
 if (state.dieselSelectedMonth) {
 var parts = state.dieselSelectedMonth.split('-');
 year = parseInt(parts[0]);
 month = parseInt(parts[1]) - 1; // 0-indexed
 
 // Para mes especifico, sempre retorna o mes inteiro
 startDate = new Date(year, month, 1, 0, 0, 0);
 // Ultimo dia do mes
 endDate = new Date(year, month + 1, 0, 23, 59, 59);
 } else {
 // Comportamento original para hoje/semana/mes atual
 ...
 // month - mes atual inteiro (do dia 1 ate hoje)
 startDate = new Date(year, month, 1, 0, 0, 0);
 }
 }
...
async function loadDieselControl() {
 try {
 // Verificar permissao de edicao e esconder/mostrar formulario
 ...
 // Preencher dropdown de meses (ultimos 12 meses)
 populateDieselMonthSelect();
 
 // Atualizar estado dos botoes de periodo
...
function populateDieselMonthSelect() {
 ...
 var html = '<option value="">Mes Especifico...</option>';
 ...
 // Se ja ha um mes selecionado, marcar
 if (state.dieselSelectedMonth) {
 select.value = state.dieselSelectedMonth;
 }
}

// Atualizar estado visual dos botoes de periodo do diesel
...
// Carregar estatisticas de diesel
async function loadDieselStats() {
 ...
 console.error('Erro ao carregar estatisticas de diesel:', error);
 }
}

// Alternar periodo do diesel
async function setDieselPeriod(period) {
 state.dieselPeriod = period;
 state.dieselSelectedMonth = null; // Limpar selecao de mes especifico
 
 // Atualizar botoes - usar IDs que comecam com diesel-filter
 ['today', 'week', 'month'].forEach(function(p) {
 var btn = document.getElementById('diesel-filter-'+ p);
 ...
 // Limpar selecao do dropdown de mes
...
// Selecionar mes especifico do diesel
async function setDieselMonth(monthValue) {
 if (!monthValue) {
 // Voltou para "mes atual"
 ...
 // Remover active de todos os botoes de periodo
 ['today', 'week', 'month'].forEach(function(p) {
 var btn = document.getElementById('diesel-filter-'+ p);
 if (btn) btn.classList.remove('active');
 });
...
function checkDieselAlerts() {
 ...
 // Limites de alerta
 var LIMITE_CRITICO = 50; // Critico - vermelho
 var LIMITE_BAIXO = 100; // Baixo - amarelo/laranja
 var LIMITE_ATENCAO = 200; // Atencao - amarelo
 ...
 '<span style="'+ subtitleStyle + 'color:#fca5a5;">Tanque vazio! Abastecimento urgente necessario.</span>'+
 '</div></div>';
 // Verificar se precisa criar requisicao de compra
 ...
 '<div style="'+ contentStyle + '">'+
 '<span style="'+ titleStyle + 'color:#f93943;">ESTOQUE CRATICO</span>'+
 '<span style="'+ subtitleStyle + 'color:#fca5a5;">Apenas <strong>'+ saldo.toLocaleString('pt-BR') + 'L</strong> restantes. Abasteca imediatamente!</span>'+
 '</div></div>';
 checkDieselAutoRequest(saldo);
 ...
 '<div style="'+ contentStyle + '">'+
 '<span style="'+ titleStyle + 'color:#eab308;">ATENA‡AƒO AO ESTOQUE</span>'+
 '<span style="'+ subtitleStyle + 'color:#fde047;"><strong>'+ saldo.toLocaleString('pt-BR') + 'L</strong> disponiveis. Monitore o consumo.</span>'+
 '</div></div>';
 } else {
 ...
 '<span style="'+ subtitleStyle + 'color:#86efac;"><strong>'+ saldo.toLocaleString('pt-BR') + 'L</strong> disponiveis no tanque.</span>'+
 '</div></div>';
 }
}

// Verificar e criar/remover requisicao automatica de diesel quando estoque baixo
...
async function checkDieselAutoRequest(saldo) {
 ...
 console.log('[Diesel] Auto-request ja em execucao, ignorando...');
 return;
 }
 
 // So usuarios com permissao podem ver/criar requisicoes
 ...
 // Procurar requisicao de diesel pendente (em analise ou pedido)
 ...
 var isAuto = p.auto_generated === true || (p.notes && p.notes.includes('AUTOMATICA'));
 return isDiesel && isPendente && isAuto;
 });
 
 if (dieselBaixo) {
 // Diesel baixo - criar requisicao se nao existe
 if (!requisicaoDiesel) {
 console.log('[Diesel] Estoque baixo ('+ saldo + 'L) - criando requisicao automatica...');
 
 var stats = state.dieselStats || {};
 var consumoMes = stats.total_saida || stats.totalSaida || 0;
 var quantidadeSugerida = Math.max(consumoMes, 200);
 
 var mensagem = 'REQUISIA‡AƒO AUTOMATICA DE DIESEL\n\n'+
 'O sistema detectou que o estoque de diesel esta baixo:\n'+
 'â€¢ Estoque atual: '+ saldo.toLocaleString('pt-BR') + 'litros\n'+
 'â€¢ Consumo do mes: '+ consumoMes.toLocaleString('pt-BR') + 'litros\n'+
 'â€¢ Quantidade sugerida: '+ quantidadeSugerida.toLocaleString('pt-BR') + 'litros\n\n'+
 'Por favor, providencie o abastecimento o mais rapido possivel.';
 ...
 if (response.ok) {
 showNotification('â›½ Requisicao de diesel criada automaticamente!', 'warning');
 console.log('[Diesel] Requisicao automatica criada com sucesso');
 // Recarregar compras
 await loadPurchases();
 }
 } else {
 console.log('[Diesel] Requisicao de diesel ja existe:', requisicaoDiesel.id);
 }
 } else {
 // Diesel OK - remover requisicao automatica se existir
 if (requisicaoDiesel) {
 console.log('[Diesel] Estoque OK ('+ saldo + 'L) - removendo requisicao automatica...');
 ...
 if (delResp.ok) {
 showNotification('Requisicao de diesel removida (estoque OK)', 'success');
 console.log('[Diesel] Requisicao automatica removida');
 await loadPurchases();
 }
 }
 }
 } catch (e) {
 console.error('[Diesel] Erro ao gerenciar requisicao automatica:', e);
 } finally {
 _dieselAutoRequestRunning = false;
 }
}

// Funcao para mostrar detalhes da requisicao
function showPurchaseDetails(purchaseId) {
 const purchase = state.purchases.find(p => p.id === purchaseId);
 if (!purchase) return;
 
 const statusConfig = {
 analise: { color: '#f59e0b', bg: 'rgba(245,158,11,0.15)', text: 'Em Analise', icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12,6 12,12 16,14"/></svg>'},
 ...
 chegando: { color: '#8b5cf6', bg: 'rgba(139,92,246,0.15)', text: 'Em Transito', icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13"/><polygon points="16,8 20,8 23,11 23,16 16,16 16,8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>'},
...
// Renderizar estatisticas do diesel
function renderDieselStats() {
 ...
 var tipoTexto = ultimo.record_type === 'entrada'? 'Entrada': 'Saida';
 ...
 elUltimaMovTipo.textContent = 'Sem registros no periodo';
 elUltimaMov.style.color = '#888';
 }
 }
}

// Renderizar grafico de diesel (barras - entradas/saidas)
function renderDieselChart() {
 var canvas = document.getElementById('diesel-consumption-chart');
 if (!canvas) return;
 
 // Verificar se Chart.js esta disponivel
 if (typeof Chart === 'undefined') {
 console.warn('[Diesel] Chart.js nao disponivel, pulando grafico');
 return;
 }
...
// Renderizar historico do diesel
function renderDieselHistory() {
 ...
 if (sorted.length === 0) {
 tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#888;">Nenhum registro no periodo</td></tr>';
 return;
 }
 
 var diasSemana = ['DOM', 'SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SAB'];
...
async function saveDieselRecord() {
 ...
 showNotification('Voce nao tem permissao para registrar diesel', 'error');
 return;
 }
...
async function deleteDieselRecord(id) {
 ...
 await loadDieselControl();
 showNotification('Registro de diesel excluido com sucesso!', 'success');
 } else {
...
// Exportar relatorio de diesel PDF
function exportDieselReportPDF() {
 ...
 state.dieselPeriod === 'week'? 'Ultima Semana': 'Ultimo Mes';
 
 var content = '<!DOCTYPE html>'+
 '<html lang="pt-BR">'+
 '<head>'+
 '<meta charset="UTF-8">'+
 '<title>Relatorio de Diesel - Granja Vitta</title>'+
 '<style>'+
 ...
 '<div class="stat-box"><h3>Total Saidas</h3><div class="value">'+ (stats.totalSaida || 0).toLocaleString('pt-BR') + 'L</div></div>'+
 '<div class="stat-box"><h3>Saldo Atual</h3><div class="value">'+ (stats.saldoAtual || 0).toLocaleString('pt-BR') + 'L</div></div>'+
 '<div class="stat-box"><h3>Media Diaria</h3><div class="value">'+ (stats.mediaDiaria || 0).toFixed(1) + 'L</div></div>'+
 '</div>'+
 '<h2 style="color:#1a1a2e;font-size:16px;margin-bottom:15px;">HISTA“RICO DE MOVIMENTAA‡A•ES</h2>'+
 '<table>'+
 '<thead><tr><th>Data</th><th>Tipo</th><th>Quantidade</th><th>Motivo</th><th>Registrado por</th><th>Observacoes</th></tr></thead>'+
 '<tbody>';
 ...
 content += '</tbody></table>'+
 '<div class="footer">'+
 '<p>Relatorio gerado automaticamente pelo Sistema Icarus | Granja Vitta</p>'+
...
// Exportar relatorio de diesel Excel/CSV
...
function generateDieselInteractiveReport(records, totalEntrada, totalSaida) {
 ...
 var typeLabel = r.record_type === 'entrada'? 'â¬† ENTRADA': 'â¬‡ SAADA';
 ...
 '<p>Granja Vitta â€¢ Relatorio gerado em '+ new Date().toLocaleDateString('pt-BR') + '</p>'+
 '</div>'+
 '<div class="stats-grid">'+
 ...
 '<div class="stat-card saida"><div class="stat-value">-'+ totalSaida.toFixed(1) + 'L</div><div class="stat-label">Total Saidas</div></div>'+
...
// Funcao auxiliar para obter datas do periodo (gerador)
...
// Carregar estatisticas de gerador
async function loadGeneratorStats() {
 ...
 console.error('Erro ao carregar estatisticas de gerador:', error);
 }
}

// Alternar periodo do gerador
...
// Renderizar estatisticas do gerador
...
// Renderizar historico do gerador
...
async function saveGeneratorRecord() {
 ...
 showNotification('Voce nao tem permissao para registrar eventos do gerador', 'error');
 return;
 }
 ...
 showNotification('Preencha os campos obrigatorios (data e tipo)', 'error');
 return;
 }
...
// Exportar relatorio de gerador PDF
function exportGeneratorReportPDF() {
 ...
 state.generatorPeriod === 'week'? 'Ultima Semana': 'Ultimo Mes';
 ...
 '<head>'+
 '<meta charset="UTF-8">'+
 '<title>Relatorio de Gerador - Granja Vitta</title>'+
 '<style>'+
 'body { font-family: Arial, sans-serif; padding: 40px; background: #fff; color: #333; }'+
 '.header { text-align: center; margin-bottom: 30px; border-bottom: 3px solid #d4af37; padding-bottom: 20px; }'+
 '.header h1 { color: #1a1a2e; margin-bottom: 5px; }'+
 ...
 '<div class="stat-box"><h3>Combustivel Usado</h3><div class="value">'+ (stats.fuelUsed || 0).toLocaleString('pt-BR') + 'L</div></div>'+
 '<div class="stat-box"><h3>Consumo Medio</h3><div class="value">'+ (stats.avgConsumption || 0).toFixed(1) + 'L/h</div></div>'+
 '<div class="stat-box"><h3>Manutencoes</h3><div class="value">'+ (stats.maintenanceCount || 0) + '</div></div>'+
 '</div>'+
 '<h2 style="color:#1a1a2e;font-size:16px;margin-bottom:15px;">HISTA“RICO DE OPERAA‡A•ES</h2>'+
 '<table>'+
 '<thead><tr><th>Data</th><th>Tipo</th><th>Tempo</th><th>Combustivel</th><th>Registrado por</th><th>Observacoes</th></tr></thead>'+
 '<tbody>';
...
function exportDashboardReport() {
 ...
 // Dados de checklists para o relatorio
 ...
 auto: c.auto_complete ? 'Sim': 'Nao',
 frequency: c.auto_complete ? (c.frequency_days === 1 ? 'Diario': c.frequency_days === 2 ? 'Dia sim/nao': `A cada ${c.frequency_days} dias`) : 'Manual'
 }))
 };
 
 // Agrupar por executor - usando assigned_users que e o formato correto
 const byExecutor = {};
 filteredOrders.forEach(o => {
 // Pegar nomes dos usuarios atribuidos
 if (o.assigned_users && Array.isArray(o.assigned_users) && o.assigned_users.length > 0) {
 o.assigned_users.forEach(user => {
 const name = user.name || user.username || 'Nao atribuido';
 ...
 // Agrupar por setor - usando 'sector'que e o campo correto
 const bySetor = {};
 filteredOrders.forEach(o => {
 const setor = o.sector || 'Nao especificado';
...
<head>
 <meta charset="UTF-8">
 <title>Relatorio Dashboard - Granja Vitta</title>
 <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
 <style>
...
<body>
 ...
 <h3>Checklists de Manutencao</h3>
 </div>
 ...
 <div style="font-size: 11px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px;">Automaticos</div>
 </div>
 ...
 <p class="footer-text">Relatorio gerado automaticamente pelo Sistema Icarus</p>
 <div class="icarus-brand">
 ...
 <span class="icarus-subtitle">Sistema Inteligente de Gestao de Manutencao</span>
 ...
 // Grafico de Executores - Estilo futurista
 ...
 // Grafico de Setores - Estilo futurista
 ...
 title: 'Relatorio Dashboard - Granja Vitta',
 type: 'table',
 content: {
 ...
 ['Pendentes', pending],
 ['Taxa de Conclusao', aproveitamento + '%'],
 ['Periodo', periodLabel]
 ]
 }
 };
 ...
 // Renderizar diretamente na pagina (funciona em APK, mobile e desktop)
 showReportInPage(htmlContent, 'Relatorio Dashboard', 'Relatorio do Dashboard gerado!', dashboardReportData);
}

// ========== DROPDOWN DE EXPORTAA‡AƒO ==========
...
'<body>'+
 ...
 '<p class="subtitle"><strong>Granja Vitta</strong> <span style="color:#64748b">â€¢</span> Sistema de Gestao</p>'+
 '<div class="period-badge">'+
 '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>'+
 periodLabel + '<span style="color:#64748b">|</span> Gerado em '+ dataGeracao + 'A s '+ horaGeracao +
 '</div>'+
 '</div>'+
 ...
 '<table>'+
 '<thead><tr><th>Nome</th><th>Setor</th><th>Itens</th><th>Automatico</th><th>Frequencia</th></tr></thead>'+
 '<tbody>'+
 ...
 '<div class="table-scroll">'+
 '<table>'+
 '<thead><tr><th>Executor</th><th>Total</th><th>Concluidas</th><th>Taxa</th></tr></thead>'+
 '<tbody>'+
 ...
 '<div class="icarus-info">'+
 '<span class="icarus-title">ICARUS SYSTEM</span>'+
 '<span class="icarus-subtitle">Sistema Inteligente de Gestao</span>'+
...
function exportAlmoxarifadoReport() {
 ...
 // Gerar relatorio HTML interativo (SEM CSV automatico)
 ...
 const htmlContent = '<!DOCTYPE html>'+
 '<html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">'+
 '<title>Relatorio Almoxarifado - Granja Vitta</title>'+
 '<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">'+
 '<script src="https://cdn.jsdelivr.net/npm/chart.js"><\/script>'+
 '<style>'+
 '*{margin:0;padding:0;box-sizing:border-box}'+
 ...
 '<div class="table-scroll"><table><thead><tr><th>SKU</th><th>Nome</th><th>Categoria</th><th>Marca</th><th>Qtd</th><th>Unidade</th><th>Localizacao</th><th>Status</th></tr></thead>'+
 ...
 // Renderizar diretamente na pagina (funciona em APK, mobile e desktop)
 showReportInPage(htmlContent, 'Relatorio Almoxarifado', 'Relatorio do Almoxarifado gerado!', almoxarifadoReportData);
}
...
async function loadAditiva() {
 try {
 // Esconder/mostrar botao de nova tarefa baseado em permissao
...
function renderAdditiveTasks() {
 ...
 var statusLabel = task.status === 'pending'? 'Pendente': task.status === 'in_progress'? 'Em Andamento': 'Concluida';
 var priorityLabel = task.priority === 'high'? 'Alta': task.priority === 'medium'? 'Media': 'Baixa';
...
// Modal para concluir tarefa aditiva com selecao de executor
...
async function confirmCompleteAdditive() {
 ...
 showNotification('Tarefa concluida!', 'success');
 } else {
...
function showNewAdditiveModal() {
 ...
 showNotification('Voce nao tem permissao para criar tarefas', 'error');
 return;
 }
...
function openAdditiveTask(taskId) {
 ...
 // Por enquanto so mostra detalhes via alert, pode expandir para modal de edicao
 var msg = 'Titulo: '+ task.title + '\n'+
...
var LAV2_CLIENTS = {
 ...
 { key: 'calca_masc', label: 'Calca Masc.'},
 { key: 'camisa_fem', label: 'Camisa Fem.'},
 { key: 'calca_fem', label: 'Calca Fem.'}
 ]
 },
 loyola: {
 id: 'client_loyola',
 ...
 fields: [
 { key: 'pecas', label: 'Pecas'},
 { key: 'marcacoes', label: 'Marcacoes', isMarking: true }
 ]
 },
 suplemento: {
...
async function loadLavanderia() {
 ...
 // So selecionar marajoara na primeira vez (se nao tiver cliente selecionado)
...
function selectLav2Client(clientId) {
 ...
 // Atualizar titulo do form
 var titleEl = document.getElementById('lav2-entry-title');
 if (titleEl) titleEl.textContent = 'Novo Lancamento';
 
 // Renderizar campos dinamicos APENAS se o cliente mudou
 if (previousClient !== clientId) {
 renderLav2FormFields(client);
 }
 
 // Calcular e mostrar periodo
...
function renderLav2FormFields(client) {
 ...
 // Mostrar/esconder preview de marcacoes
...
async function loadLav2AllStats() {
 ...
 // Total de pecas geral (todos os clientes)
...
function renderLav2History() {
 ...
 '<polyline points="14 2 14 8 20 8"/>'+
 '</svg>'+
 '<div style="font-size: 14px; margin-bottom: 4px;">Nenhum lancamento</div>'+
 '<div style="font-size: 12px;">Adicione o primeiro lancamento do periodo</div>'+
 '</div>';
 return;
 }
 ...
 '<div style="font-size: 12px; color: rgba(255,255,255,0.5);">'+ totalPieces + 'pecas</div>'+
 '</div>'+
 '<div style="display: flex; align-items: center; gap: 12px;">'+
...
async function saveLav2Entry() {
 ...
 var data = await response.json();
 if (data.ok) {
 showToast('Lancamento salvo!', 'success');
...
async function exportLav2PDF() {
 ...
 '<title>Relatorio '+ client.name + '</title>'+
 '<style>'+
 ...
 '</style></head><body>'+
 '<div class="header">'+
 '<h1>Relatorio de Lavanderia - '+ client.name + '</h1>'+
 '<div class="period">Periodo: '+ formatDate(periodStart) + 'a '+ formatDate(periodEnd) + '</div>'+
 '</div>'+
 '<div class="summary">'+
 '<div class="summary-item"><div class="value">'+ totalPieces + '</div><div class="label">Total de Pecas</div></div>'+
 (client.markingPrice > 0 ? '<div class="summary-item"><div class="value">'+ totalMarkings + '</div><div class="label">Marcacoes</div></div>': '') +
 ...
 '<div class="footer">'+
 '<p>Gerado por ICARUS - Sistema de Gestao</p>'+
...
async function loadRelatorios() {
 try {
 // Esconder/mostrar botao de novo relatorio baseado em permissao
 ...
 // Inicializar aba de Notas & Boletos se usuario tiver permissao
 initNotasTab();
...
function renderReports() {
 ...
 // Se ha um relatorio aberto, nao esconder o viewer
 if (state.currentReport) {
 // Manter viewer visivel, apenas atualizar dados internos se necessario
 } else {
 ...
 '<p>Seja o primeiro a publicar um relatorio!</p>'+
 '</div>';
 return;
 }
...
// Funcao para formatar data relativa
...
function openReport(reportId) {
 ...
 var categoryLabels = { 'geral': 'Geral', 'manutencao': 'Manutencao', 'incidente': 'Incidente', 'melhoria': 'Melhoria', 'orcamento': 'Orcamento'};
 ...
 document.getElementById('viewer-author').textContent = report.created_by_name || 'Anonimo';
 document.getElementById('viewer-content').textContent = report.content || '';
 
 // Analise inteligente do conteudo
 ...
 visibilityEl.textContent = 'PUBLICO';
 ...
 // Mostrar/esconder botao de deletar baseado em permissao (dono ou admin)
...
function copyReportToClipboard() {
 ...
 showNotification('Conteudo copiado!', 'success');
 }).catch(function() {
...
// Analise inteligente do conteudo - detecta valores, quantidades, datas
function analyzeReportContent(content) {
 ...
 // Detectar quantidades (X unidades, X pecas, X un, Qtd: X)
 var qtyRegex = /(\d+)\s*(unidade|unidades|un|peca|pecas|pc|pcs|qtd|quantidade)/gi;
...
// Renderizar analise inteligente
...
// Funcao para gerar PDF premium via backend
...
// Funcao para imprimir relatorio como PDF - Design Premium Magenta
function printReport() {
 ...
 printWindow.document.write('<div class="logo-text"><div class="logo-title">ICARUS</div><div class="logo-subtitle">Sistema de Gestao Inteligente</div></div>');
 printWindow.document.write('</div>');
 printWindow.document.write('<div class="header-date">Central de Relatorios<br>Granja Vitta</div>');
 printWindow.document.write('</div>');
 ...
 printWindow.document.write('<div><div class="meta-name">'+ (report.created_by_name || 'Anonimo') + '</div><div class="meta-role">Equipe de Manutencao</div></div>');
 printWindow.document.write('</div>');
 ...
 printWindow.document.write('<div class="promo-subtitle">Gestao Inteligente de Manutencao â€¢ Granja Vitta</div>');
 ...
 printWindow.document.write('<div class="footer-text">Relatorio gerado pelo Sistema Icarus em '+ new Date().toLocaleDateString('pt-BR') + '</div>');
...
async function deleteReport() {
 ...
 if (!isOwner && !isAdmin) {
 showNotification('Voce nao tem permissao para excluir este relatorio', 'error');
 return;
 }
 
 if (!confirm('Tem certeza que deseja excluir este relatorio permanentemente?')) return;
 ...
 await loadReportsData();
 renderReports();
 showNotification('Relatorio excluido com sucesso!', 'success');
 } else {
 showNotification(data.error || 'Erro ao excluir relatorio', 'error');
 }
 } catch (error) {
 console.error('Erro ao excluir relatorio:', error);
 showNotification('Erro ao excluir relatorio', 'error');
 }
}

// Array temporario para anexos
var reportAttachments = [];

function showNewReportModal() {
 if (!state.canWriteRelatorios) {
 showNotification('Voce nao tem permissao para criar relatorios', 'error');
 return;
 }
...
function addReportAttachment() {
 ...
 if (!url) {
 showNotification('Cole um link valido', 'error');
 return;
 }
...
async function saveReport() {
 ...
 if (!title || !content) {
 showNotification('Titulo e conteudo sao obrigatorios', 'error');
 return;
 }
 ...
 closeReportModal();
 renderReports();
 showNotification('Relatorio publicado com sucesso!', 'success');
 } else {
 showNotification(data.error || 'Erro ao publicar relatorio', 'error');
 }
 } catch (error) {
 console.error('Erro ao salvar relatorio:', error);
 showNotification('Erro ao salvar relatorio', 'error');
 }
}
...
// ============================================
// SISTEMA DE DIARIAS - GUILHERME BRAGA
// ============================================
...
function renderDiariasSemanas() {
 ...
 // Sabado e -2 dias a partir de segunda, os outros sao 0,1,2,3,4
...
function confirmAddSemana() {
 ...
 let selectedDate = new Date(dateInput.value + 'T12:00:00'); // Adiciona horario para evitar problemas de timezone
 
 // Ajustar para segunda-feira mais proxima (anterior)
 ...
 // Verificar se ja existe uma semana com essa data
 const exists = diariasData.semanas.some(s => s.startDate === startDate);
 if (exists) {
 showNotification('Ja existe uma semana com essa data', 'warning');
...
function generateDiariasPDF() {
 ...
 // Estrela de Davi SVG (hexagrama - dois triangulos sobrepostos)
 ...
 <html>
 <head>
 <title>Relatorio de Diarias - Guilherme Braga</title>
 <style>
 * { margin: 0; padding: 0; box-sizing: border-box; }
 ...
 <p class="time">Gerado A s ${horaGeracao}</p>
 </div>
 </div>
 ...
 <p>Relatorio de Diarias â€¢ Valor: R$ ${diariasData.valorDiaria.toFixed(2)}/dia</p>
 </div>
 </div>
...
// Exportar Diarias como HTML interativo com animacoes
...
'<meta charset="UTF-8">'+
'<meta name="viewport" content="width=device-width, initial-scale=1.0">'+
'<title>Diarias - Guilherme Braga | ICARUS</title>'+
'<style>'+
'@import url("https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap");'+
'* { margin: 0; padding: 0; box-sizing: border-box; }'+
...
'</div>'+
'<div class="logo-text"><h1>ICARUS</h1><p>Controle de Diarias</p></div>'+
'</div>'+
...
'</div>'+
'<div class="date-badge"><div class="date">'+ dataGeracao + '</div><div class="time">Gerado A s '+ horaGeracao + '</div></div>'+
'</div>'+
'</div>'+
...
// ============================================
// SISTEMA DE NOTAS & BOLETOS - GESTAƒO FINANCEIRA
// ============================================
...
// Verificar se usuario tem permissao para ver Notas & Boletos
function canAccessNotas() {
 ...
 // Se role contem 'manut'ou e admin/owner, tem acesso
 ...
 // Verificar por nome de usuario especifico
...
function initNotasTab() {
 ...
 // So resetar para Forum se for a primeira vez (nao se ja estiver em notas)
...
function switchRelatoriosTab(tab) {
 ...
 forumContent.style.display = 'block';
 notasContent.style.display = 'none';
 // Mostrar header do forum, botao novo post e stats
 ...
 // Esconder header do forum, botao novo post e stats na aba Notas
...
// ============================================
// RELATA“RIO DE GASTOS DE MANUTENA‡AƒO
// ============================================

// Estado do relatorio
let gastosReportVisible = false;

// Mostrar tela de relatorio de gastos (substitui a area de notas)
async function showGastosReport() {
 const notasContent = document.getElementById('notas-content');
 if (!notasContent) return;
 
 // Salvar conteudo original para restaurar depois
...
// Calcular estatisticas localmente (fallback)
function calculateLocalStats() {
 ...
 // Por setor
 const setor = item.setor || 'Nao informado';
 ...
 // Por empresa
 const empresa = item.empresa || 'Nao informado';
...
// Renderizar view de relatorio (dentro do container de notas)
function renderGastosReportView(container, stats) {
 ...
 <p style="font-size: 13px; color: rgba(255,255,255,0.5); margin: 0;">Controle de manutencao e despesas</p>
 </div>
 </div>
 ...
 `;
 }).join('') : '<p style="color: rgba(255,255,255,0.4); text-align: center; width: 100%; padding: 40px 0;">Nenhum dado disponivel</p>'}
 </div>
 </div>
 </div>
 ...
 <p style="font-size: 12px; color: rgba(255,255,255,0.3);">Relatorio gerado em ${new Date().toLocaleString('pt-BR')} â€¢ ICARUS Sistema de Manutencao</p>
 </div>
 `;
}
...
function exportGastosReportPDF() {
 ...
 <html>
 <head>
 <title>Relatorio de Gastos - ICARUS</title>
 <style>
 @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap');
 * { margin: 0; padding: 0; box-sizing: border-box; }
 body { 
...
function renderNotas() {
 ...
 <h3 style="font-size: 16px; font-weight: 600; color: #fff; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${escapeHtml(item.empresa || 'Empresa nao informada')}</h3>
 ...
 <p style="font-size: 13px; color: rgba(255,255,255,0.5); margin: 0 0 10px 0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${escapeHtml(item.descricao || 'Sem descricao')}</p>
...
function openNovaNotaModal(editId = null) {
 ...
 <input type="text" id="nota-empresa" value="${item?.empresa || ''}" required placeholder="Ex: Auto Pecas Silva, Eletro Center..." style="width: 100%; padding: 14px 16px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: #fff; font-size: 14px;" oninput="smartFillNota(this.value)">
 </div>
 
 <div>
 <label style="display: block; font-size: 12px; color: rgba(255,255,255,0.6); margin-bottom: 6px;">Responsavel pelo Servico</label>
 ...
 <option value="Galpao 1" ${item?.setor === 'Galpao 1'? 'selected': ''}>Galpao 1</option>
 <option value="Galpao 2" ${item?.setor === 'Galpao 2'? 'selected': ''}>Galpao 2</option>
 <option value="Galpao 3" ${item?.setor === 'Galpao 3'? 'selected': ''}>Galpao 3</option>
 <option value="Expedicao" ${item?.setor === 'Expedicao'? 'selected': ''}>Expedicao</option>
 <option value="Cantina" ${item?.setor === 'Cantina'? 'selected': ''}>Cantina</option>
 <option value="Escritorio" ${item?.setor === 'Escritorio'? 'selected': ''}>Escritorio</option>
 <option value="Oficina" ${item?.setor === 'Oficina'? 'selected': ''}>Oficina</option>
 <option value="Fabrica de Racao" ${item?.setor === 'Fabrica de Racao'? 'selected': ''}>Fabrica de Racao</option>
 ...
 <!-- Secao: Descricao -->
 <div style="margin-bottom: 24px;">
 <label style="display: block; font-size: 12px; color: rgba(255,255,255,0.6); margin-bottom: 6px;">Descricao do Servico/Produto *</label>
 <textarea id="nota-descricao" required placeholder="Descreva o servico prestado ou produtos adquiridos..." style="width: 100%; padding: 14px 16px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: #fff; font-size: 14px; min-height: 80px; resize: vertical;">${item?.descricao || ''}</textarea>
 </div>
 ...
 <div>
 <label style="display: block; font-size: 12px; color: rgba(255,255,255,0.6); margin-bottom: 6px;">Data de Emissao</label>
 ...
 ${isEdit ? 'Salvar Alteracoes': 'Cadastrar Entrada'}
 </button>
 </div>
 </form>
 </div>
...
// Variaveis temporarias para anexos
...
// Smart fill - sugestoes baseadas em entradas anteriores
...
async function saveNota(event, editId) {
 ...
 showNotification('Preencha a empresa e descricao', 'warning');
 return;
 }
...
function viewNota(id) {
 ...
 statusColor = '#ef4444';
 statusText = 'Vencido ha '+ Math.abs(diasVenc) + 'dias';
 } else if (diasVenc !== null && diasVenc <= 7) {
 statusColor = '#fb923c';
 ...
 <p style="font-size: 13px; color: rgba(255,255,255,0.5); margin: 0;">${escapeHtml(item.setor || 'Setor nao informado')}</p>
 </div>
 ...
 <p style="font-size: 10px; color: rgba(255,255,255,0.4); text-transform: uppercase; margin: 0 0 4px 0;">Responsavel</p>
 ...
 <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
 ${item.status === 'pago'? 'Ja Pago': 'Marcar como Pago'}
 </button>
 </div>
 </div>
 </div>
...
async function deleteNota(id) {
 ...
 updateNotasStats();
 closeViewNotaModal();
 showNotification('Entrada excluida', 'success');
 } else {
 ...
 updateNotasStats();
 closeViewNotaModal();
 showNotification('Excluido localmente', 'warning');
 }
}

// ============================================
// EXPORTAA‡AƒO DE NOTAS - PDF E HTML PREMIUM
// ============================================
...
function generateNotaHTMLContent(item) {
 ...
 html += '<h1>ICARUS</h1>\n';
 html += '<p>Sistema de Gestao Premium</p>\n';
 html += '</div>\n';
 html += '</div>\n';
 html += '\n';
 ...
 html += '<div class="doc-date">Emitido em '+ dataEmissao + 'A s '+ horaEmissao + '</div>\n';
 html += '</div>\n';
 ...
 html += ''+ escapeHtml(item.setor || 'Setor nao informado') + '\n';
 html += '</div>\n';
 html += '</div>\n';
 ...
 html += '<h3>Descricao do Servico/Produto</h3>\n';
 ...
 html += '<div class="label">Responsavel</div>\n';
 html += '<div class="value">'+ escapeHtml(item.responsavel) + '</div>\n';
 html += '</div>\n';
 }
 if (item.data_emissao) {
 html += '<div class="info-item">\n';
 html += '<div class="label">Data de Emissao</div>\n';
...
function exportNotaPDF(id) {
 ...
 showNotification('Preparando PDF para impressao...', 'success');
}

// Inicializar quando entrar na view de relatorios
...
// Tambem inicializar se ja estiver na view
document.addEventListener('DOMContentLoaded', function() {
 setTimeout(() => {
